...
dataset_name = sys.argv[1] #'ssc.hg38'
				...
				for opt_start_index, start_index, end_index, opt_end_index in pull_deletion_indices(chrom_states[:, anc]):
					...
					if majority_parental_inheritance is not None and np.all(majority_parental_inheritance!=-1):
						...
						if len(trans) + len(notrans) == family_size-2:
							deletions.append(Deletion(sibpair['family'], chrom,
...
with open('%s/individuals.json' % dataset_name, 'w+') as f:
	json.dump(sorted(individuals), f, indent=4)
with open('%s/children.json' % dataset_name, 'w+') as f:
	json.dump(sorted(children), f, indent=4)
with open('%s/families.json' % dataset_name, 'w+') as f:
...
with open('%s/deletions.json' % dataset_name, 'w+') as f:
