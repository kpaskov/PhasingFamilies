...
def pull_upd(family_id, states, chroms, starts, ends, individuals,  indices, other_indices, is_mat):
    ...
    for chrom in chroms_of_interest:
        ...
        for individual, index, other_index in zip(individuals[2:], indices[2:], other_indices[2:]):
...
for sibpair in sibpairs:
	...
	if len(missing_chroms)>0:
		print('missing %s' % str(missing_chroms))
		...
		mat_upds = pull_upd(family_key, states, chroms, starts, ends, individuals, mat_indices, pat_indices, True)
		pat_upds = pull_upd(family_key, states, chroms, starts, ends, individuals, pat_indices, mat_indices, False)
