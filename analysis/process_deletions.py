...
# pull twins
child_to_identicals = dict()
with open(identicals_file, 'r') as f:
	next(f) # skip header
	for line in f:
		pieces = line.strip().split('\t')
		for child in pieces:
			child_to_identicals[child] = pieces
print('identicals', len(child_to_identicals))

# pull families and figure out how many non-identical kids they have
family_to_individuals = dict()
family_to_num_kids = dict()
...
for j in family_sizes:
	with open('%s/chr.%s.familysize.%d.families.txt' % (phase_dir, chrom, j), 'r') as f:
		...
		for line in f:
			...
			# figure out family size, to be used for bayesian estimates (identical twins count as 1)
			num_kids = 0
			kids_to_skip = set()
			for ind in individuals[2:]:
				if ind not in kids_to_skip:
					num_kids += 1
					if ind in child_to_identicals:
						kids_to_skip.update(child_to_identicals[ind])
			family_to_num_kids[family_key] = num_kids
...
Deletion = namedtuple('Deletion', ['family', 'chrom', 'start_pos', 'end_pos', 'length', 'opt_start_pos', 'opt_end_pos', 
	'trans', 'notrans', 'family_size', 'is_mat', 'is_pat'])
			...
			if ok_region and inh_known and (not has_recomb):
				...
				if (len(trans) + len(notrans) == j-2) and (len(trans) > 0):
					deletions.append(Deletion(family_key, chrom, int(start_pos), int(end_pos), int(end_pos-start_pos+1),
						...
						2+family_to_num_kids[family_key], is_mat, is_pat))
