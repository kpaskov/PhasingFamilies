...
data_dir = '/oak/stanford/groups/dpwall/users/kpaskov/DATA/ssc.hg38'
ped_file = 'ssc.ped'
...
child_to_phen = dict()
with open('%s/%s' % (data_dir, ped_file), 'r') as f:
    for line in f:
...
# pull only unaff children
children = [x for x in children if child_to_phen[x]=='1']
...
total = 0
with gzip.open('possible_denovo_ssc.txt.gz', 'wt') as f:
